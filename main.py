...
def download_csv(df, label, filename):
    ...
    if sow_file:
        try:
            ...
            if st.button("Run Smart Turnkey Estimation"):
                ...
                summary_df = pd.DataFrame.from_dict({k: v for k, v in result.items() if isinstance(v, str)}, orient="index", columns=["Value"])
                st.table(summary_df)
                st.json(result)
    ...
    hist_contract_value = st.number_input("Contract Value (SAR)", min_value=0)
