...
if mode == "Turnkey Cost Estimation":
    ...
    if sow_file and bid_file:
        try:
            if bid_file.name.endswith("csv"):
                bid_df = pd.read_csv(bid_file)
            else:
                bid_df = pd.read_excel(bid_file)
            st.dataframe(bid_df.head())
            st.warning("Bid Price Analysis engine is under development.")
        except Exception as e:
            st.error(f"Error reading bid file: {e}")
    else:
    ...
    if hist_file:
        try:
            ...
            st.success("File uploaded and stored successfully for future reference.")
