        ...
        with st.spinner("Processing file..."):
            try:
                ...
                if len(text.strip().split()) < 50:
                    st.error("The uploaded file seems too short. Please upload a full Scope of Work document.")
                else:
    ...
    if "sow_text" in st.session_state and st.session_state.get("is_valid"):
        ...
        if inquiries:
            ...
            for i, iq in enumerate(inquiries):
                st.markdown(f"- {'❗' if iq['critical'] else 'ℹ️'} **{iq['text']}**")
            ...
            with st.spinner("Running estimation..."):
                try:
                    ...
                    if result.get("Used Historical Model") is False:
                        st.warning("No historical data was used — results may be less accurate.")
                    if result.get("Model Confidence") and float(str(result["Model Confidence"]).replace('%', '')) < 80:
                        st.warning("The AI model is not fully confident in the results — review is recommended.")
