...
class CostimAIzeOrchestrator:
    ...
    def extract_text(self, file):
        """Extracts text from uploaded file (PDF, DOCX, TXT) with encoding detection."""
        try:
            content = file.read()
    ...
    def validate_sow(self, text):
        """Validates the scope of work based on presence of key phrases."""
    ...
    def detect_inquiries(self, text):
        ...
        if "gis" in text_lower and "bays" not in text_lower:
            inquiries.append({"text": "GIS mentioned without specifying number of bays - impacts cost estimation.", "critical": True})
        if "transformer" in text_lower and "capacity" not in text_lower and "rating" not in text_lower:
            inquiries.append({"text": "Transformers mentioned without capacity or rating - required for accurate cost.", "critical": True})
        if "substation" in text_lower and "voltage" not in text_lower:
            inquiries.append({"text": "Substation mentioned without voltage level - affects material and labor costs.", "critical": True})
        if "large" in text_lower and "area" not in text_lower and "size" not in text_lower:
            inquiries.append({"text": "Large project mentioned without specific area/size - needed for scale estimation.", "critical": False})
        if "supply" in text_lower and "install" not in text_lower and "installation" not in text_lower:
            inquiries.append({"text": "Supply mentioned but installation scope unclear - impacts labor cost.", "critical": False})

        return inquiries
    
    def run_estimation(self, text):
        """Generates cost estimate based on analyzed features."""
        # فحص ما إذا تم التقدير مسبقًا خلال أقل من 90 يومًا
        hash_key = hashlib.md5(text.encode()).hexdigest()
        log_file = "/mnt/data/os.path.join(os.getcwd(), "estimation_log.csv")"
...
try:
    ...
    def archive_historical_prices(self, file, metadata=None):
        """Processes historical data file and trains internal model."""
        try:
            import pandas as pd
try:
    import openpyxl
